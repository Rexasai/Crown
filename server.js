...
app.post("/api/register", async (req, res) => {
  const { growId, name, password, confirmPassword, referral, email, phone } = req.body || {};
...
app.post("/api/admin/economy", authMiddleware, adminMiddleware, async (req, res) => {
  const { ticketPrice, gachaCost, minBalanceTopup, ownerWhatsApp } = req.body || {};
  await db.write((d) => {
...
app.post("/api/admin/gacha", authMiddleware, adminMiddleware, async (req, res) => {
  ...
  const box = { id: newid("BOX"), name: String(name).trim(), icon: icon || "🎁", rewards: [] };
