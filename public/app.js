...
// Device ID: dipakai server buat cek "1 HP cuma 1x klaim ticket gratis".
// Ini best-effort saja (tersimpan di localStorage) -- bukan proteksi anti-cheat
// yang tidak bisa ditembus, karena localStorage bisa dihapus/incognito bisa
// beda ID tiap sesi. Tapi cukup buat mencegah multi-akun casual.
function getDeviceId() {
  let id = localStorage.getItem("crown_device");
  if (!id) {
    id = (window.crypto && crypto.randomUUID) ? crypto.randomUUID() : "dev_" + Date.now().toString(36) + Math.random().toString(36).slice(2);
    localStorage.setItem("crown_device", id);
  }
  return id;
}
...
// Icon item bisa berupa emoji (teks biasa) ATAU gambar PNG/JPG yang diupload admin
// (path /uploads/xxx.png). Dua helper ini dipakai di semua tempat yang nampilin icon.
const isImgIcon = (icon) => typeof icon === "string" && /^(\/uploads\/|https?:\/\/)/.test(icon);
const renderIcon = (icon, fallback) => (isImgIcon(icon) ? `<img src="${icon}" alt="" class="icon-img">` : esc(icon || fallback || "🎁"));
function iconField(idPrefix, currentValue, label) {
  label = label || "ICON / GAMBAR";
  const img = isImgIcon(currentValue) ? currentValue : "";
  return `<label>${label}
    <div class="iconfield">
      <input id="${idPrefix}_icon" value="${esc(currentValue || "")}" placeholder="emoji, atau upload gambar →">
      <input type="file" accept="image/png,image/jpeg,image/webp" id="${idPrefix}_iconfile" class="hide" onchange="uploadIconFor('${idPrefix}')">
      <button type="button" class="btn" onclick="document.getElementById('${idPrefix}_iconfile').click()">📤 PNG</button>
    </div>
    <div id="${idPrefix}_iconpreview">${img ? `<img src="${img}" class="icon-preview" alt="">` : ""}</div>
  </label>`;
}
async function uploadIconFor(prefix) {
  const fileInput = document.getElementById(prefix + "_iconfile");
  ...
  try {
    ...
    const r = await api("/admin/upload-icon", { method: "POST", body: fd, isForm: true });
    document.getElementById(prefix + "_icon").value = r.url;
    document.getElementById(prefix + "_iconpreview").innerHTML = `<img src="${r.url}" class="icon-preview" alt="">`;
    toast("Gambar terupload ✅");
...
function home() {
  ...
  <div class="realmcard" onclick="hostServer()">
    <div class="realmcard-body"><div class="eyebrow">📖 HOW TO PLAY</div><h2>🖥️ Host Server</h2><p class="muted">Cara connect ke server dari PC, Mac, iOS & Android.</p></div>
    <div class="realmcard-arrow">→</div>
  </div>
  <div class="joinrow">
    <div class="joinbtn" onclick="joinPopup('wa')">${renderIcon(STATE.settings.whatsappIcon, "💬")}<span>WhatsApp</span></div>
    <div class="joinbtn" onclick="joinPopup('dc')">${renderIcon(STATE.settings.discordIcon, "🎮")}<span>Discord</span></div>
  </div>`;
}
function joinPopup(type) {
  const isWa = type === "wa";
  const link = isWa ? STATE.settings.whatsappGroupLink : STATE.settings.discordInviteLink;
  const label = isWa ? "WhatsApp Community" : "Discord Server";
  const icon = renderIcon(isWa ? STATE.settings.whatsappIcon : STATE.settings.discordIcon, isWa ? "💬" : "🎮");
  if (!link) return toast(`Link ${label} belum diset admin.`);
  document.getElementById("modalbody").innerHTML = `<div style="text-align:center">
    <div class="spinner" style="height:auto;font-size:52px;margin:6px 0">${icon}</div>
    <h2>Gabung ${label} CROWN PS!</h2>
    <p class="muted">Yuk join biar nggak ketinggalan info event, giveaway, promo top up, dan bantuan langsung dari admin & sesama player.</p>
    <button class="primary full" onclick="window.open('${esc(link)}','_blank');closeModal()">✅ JOINED</button>
  </div>`;
  document.getElementById("modal").classList.remove("hide");
}

// ====== Host Server (How to Play) ======
function hostsSnippet() {
  const ip = STATE.settings.serverIp || "IP_SERVER_KAMU";
  return [`${ip} growtopia1.com`, `${ip} growtopia2.com`, `${ip} www.growtopia1.com`, `${ip} www.growtopia2.com`, `${ip} growtopiagame.com`].join("\n");
}
function copyHosts() {
  navigator.clipboard.writeText(hostsSnippet()).then(() => toast("Disalin ✅")).catch(() => toast("Gagal menyalin, salin manual ya."));
}
function hostServer() {
  document.getElementById("content").innerHTML = `<button class="btn" onclick="home()">← Kembali</button>
  <div class="hero" style="margin-top:12px"><div class="eyebrow">📖 HOW TO PLAY</div><h1>${esc(STATE.settings.serverName || "Server")} Host</h1><div class="notice" style="text-align:left">IP Server: <b>${esc(STATE.settings.serverIp || "Belum diset admin")}</b></div></div>
  <div class="admintabs" id="hostTabs">
    <button class="admintab active" data-p="pc" onclick="hostTab('pc')">🖥️ PC</button>
    <button class="admintab" data-p="mac" onclick="hostTab('mac')">🍎 Mac</button>
    <button class="admintab" data-p="ios" onclick="hostTab('ios')">📱 iOS</button>
    <button class="admintab" data-p="android" onclick="hostTab('android')">🎮 Android</button>
  </div>
  <div id="hostBody"></div>`;
  hostTab("pc");
}
function hostTab(platform) {
  document.querySelectorAll("#hostTabs .admintab").forEach((b) => b.classList.toggle("active", b.dataset.p === platform));
  const snippetHTML = `<div class="card hostcard"><pre class="hostsnippet">${esc(hostsSnippet())}</pre><button class="btn full" onclick="copyHosts()">⧉ COPY</button></div>`;
  const ipHTML = `<div class="card hostcard" style="text-align:center"><b style="font-size:16px">${esc(STATE.settings.serverIp || "Belum diset admin")}</b></div>`;
  const steps = {
    pc: [
      `Tekan <b>Win+R</b>, ketik <code>notepad</code>, klik kanan → <b>Run as administrator</b>`,
      `Buka file: <code>C:\\Windows\\System32\\drivers\\etc\\hosts</code>`,
      `Tambahkan baris berikut di paling bawah:${snippetHTML}`,
      `Simpan dengan <b>Ctrl+S</b>, lalu tutup Notepad`,
      `Buka Growtopia dan mainkan! 🎮`,
    ],
    mac: [
      `Buka <b>Terminal</b> (Spotlight → ketik "Terminal")`,
      `Ketik <code>sudo nano /etc/hosts</code>, masukkan password Mac kamu`,
      `Tambahkan baris berikut di paling bawah:${snippetHTML}`,
      `Simpan dengan <b>Ctrl+O</b> lalu <b>Enter</b>, keluar dengan <b>Ctrl+X</b>`,
      `Buka Growtopia dan mainkan! 🎮`,
    ],
    ios: [
      `Install aplikasi DNS/proxy changer dari App Store (misal Surge, Shadowrocket, atau sejenisnya)`,
      `Tambahkan rule custom yang mengarahkan domain growtopia1.com, growtopia2.com, www.growtopia1.com, www.growtopia2.com, growtopiagame.com ke IP:${ipHTML}`,
      `Aktifkan profil/VPN-nya`,
      `Buka Growtopia dan mainkan! 🎮`,
    ],
    android: [
      `Install aplikasi DNS/proxy changer dari Play Store (misal HTTP Injector, Postern, atau sejenisnya)`,
      `Tambahkan rule custom yang mengarahkan domain growtopia1.com, growtopia2.com, www.growtopia1.com, www.growtopia2.com, growtopiagame.com ke IP:${ipHTML}`,
      `Aktifkan VPN/profilnya`,
      `Buka Growtopia dan mainkan! 🎮`,
    ],
  };
  document.getElementById("hostBody").innerHTML = steps[platform].map((s, i) => `<div class="hoststep"><span class="hoststep-num">${i + 1}</span><div>${s}</div></div>`).join("");
}

// ====== Inventory ======
function itemRow(x) {
  return `<div class="item"><div class="itemicon">${renderIcon(x.icon)}</div><div class="grow"><b>${esc(x.name)}</b><div class="muted">${esc(x.rarity)} • Qty ${x.qty || 1} • ${esc(x.status || "Available")}</div></div><button class="btn" onclick="take('${encodeURIComponent(x.key)}')">DETAIL</button></div>`;
}
function inventory() {
...
function cellHTML(r, idx) {
  ...
  const icon = renderIcon(r.icon, isZonk(r) ? "💀" : "🎁");
...
function renderGachaBox(g) {
  const legend = g.rewards.map((r, i) => `<div class="legendrow"><span class="dot" style="background:${colorFor(r.rarity, i)}"></span>${renderIcon(r.icon)} ${esc(r.name)} <span class="muted">(${esc(r.rarity)} • ${r.chance}%)</span></div>`).join("");
  const bannerHTML = g.banner ? `<div class="gachabanner" style="background-image:linear-gradient(180deg,transparent,#100e18f0),url('${esc(g.banner)}')"></div>` : "";
  return `<div class="card wheelcard">
    ${bannerHTML}
    <h3>${renderIcon(g.icon)} ${esc(g.name)}</h3>
...
function gachaBannerHTML(g) {
  const bgStyle = g.banner ? ` style="background-image:linear-gradient(180deg,#08070f10,#0b0a13ee),url('${esc(g.banner)}');background-size:cover;background-position:center"` : "";
  return `<div class="realmcard"${bgStyle} onclick="openGachaBox('${g.id}')">
    <div class="realmcard-body">
      <div class="eyebrow">${g.rewards.length} ITEM</div>
      <h2>${renderIcon(g.icon)} ${esc(g.name)}</h2>
...
async function roll(gachaId, count) {
  ...
  setTimeout(async () => {
    ...
    const icon = renderIcon(result.icon, zonk ? "💀" : "🎁");
...
function showMultiResults(results, level) {
  ...
  document.getElementById("modalbody").innerHTML = `<div class="spinbox">
    ...
    <div class="multiresults">${results.map((r, i) => {
      ...
      const icon = renderIcon(r.icon, zonk ? "💀" : "🎁");
...
// ====== Top Up ======
function topup() {
  document.getElementById("content").innerHTML = `<div class="hero"><div class="eyebrow">TOP UP</div><h1>Top Up 💳</h1><p>Pilih apakah ingin menambah Balance atau Convert Saldo jadi Ticket.</p></div><div class="grid"><div class="card"><h2>💰 Top Up Saldo</h2><p class="muted">Minimum ${money(STATE.settings.minBalanceTopup)}. Nominal bebas di atas minimum.</p><input id="topAmount" type="number" min="${STATE.settings.minBalanceTopup}" step="1000" placeholder="Contoh: 50000"><select id="topMethod"><option value="">Pilih metode</option><option>QRIS</option></select><button class="primary full" onclick="continueBalance()">CONTINUE</button></div>
  <div class="card">
    <h2>🎟️ Convert Ticket</h2>
    <p class="muted">1 Ticket = ${money(STATE.settings.ticketPrice)} • Saldo kamu: <b>${money(ME.balance)}</b></p>
    <label>SALDO
      <div class="row" style="gap:8px">
        <input id="cvSaldo" type="number" min="0" step="1000" placeholder="0" style="flex:1">
        <button type="button" class="btn" onclick="setMaxConvert()" style="flex:none;padding:11px 14px">MAX</button>
      </div>
    </label>
    <div class="convertx">✕</div>
    <label>TICKET<input id="cvTicket" value="0" disabled></label>
    <div class="muted" id="cvNote" style="margin:-4px 0 10px"></div>
    <button class="primary full" onclick="doConvert()">CONVERT</button>
  </div></div>`;
  const saldoInput = document.getElementById("cvSaldo");
  saldoInput.oninput = updateConvertPreview;
  updateConvertPreview();
}
function updateConvertPreview() {
  const price = STATE.settings.ticketPrice || 1;
  const saldo = Math.max(0, +document.getElementById("cvSaldo").value || 0);
  const qty = Math.floor(saldo / price);
  document.getElementById("cvTicket").value = qty;
  const sisa = saldo - qty * price;
  document.getElementById("cvNote").textContent = sisa > 0 ? `Sisa ${money(sisa)} tidak terpakai (kurang dari harga 1 ticket).` : "";
}
function setMaxConvert() {
  document.getElementById("cvSaldo").value = ME.balance;
  updateConvertPreview();
}
async function doConvert() {
  ...
  const saldo = Math.max(0, +document.getElementById("cvSaldo").value || 0);
  const qty = Math.floor(saldo / price);
  if (qty < 1) return toast("Saldo belum cukup buat 1 ticket.");
  try {
    await api("/tickets/buy", { method: "POST", body: { qty } });
    await refreshMe();
    toast(`+${qty} Ticket • -${money(qty * price)} Saldo`);
    topup();
  } catch (e) { toast(e.message); }
}
...
function viewPlayer(id) {
  ...
  ${inv.length ? inv.map((x) => `<div class="item"><div class="itemicon">${renderIcon(x.icon)}</div><div class="grow"><b>${esc(x.name)}</b><div class="muted">${esc(x.rarity)} • Qty ${x.qty || 1} • ${esc(x.status || "Available")}</div></div></div>`).join("") : `<div class="empty">Inventory kosong.</div>`}`;
  document.getElementById("modal").classList.remove("hide");
}
...
function renderAdminSettings(body) {
  ...
  <div class="card">
    <label>NOMOR WHATSAPP OWNER (buat DM setelah top up)<input id="aw" value="${esc(s.ownerWhatsApp)}"></label>
  ...
  <h2 style="margin-top:18px">🖥️ Dashboard Host Server</h2>
  <div class="card">
    <label>NAMA SERVER<input id="asServerName" value="${esc(s.serverName || "")}" placeholder="CROWN PS"></label>
    <label>IP SERVER<input id="asServerIp" value="${esc(s.serverIp || "")}" placeholder="Contoh: 51.83.253.95"></label>
    <div class="notice" style="text-align:left">IP ini dipakai buat isi otomatis instruksi "How to Play" (PC/Mac/iOS/Android) yang dilihat player di Dashboard.</div>
  </div>
  <h2 style="margin-top:18px">💬 Join Community</h2>
  <div class="card">
    <label>LINK GRUP WHATSAPP<input id="asWaLink" value="${esc(s.whatsappGroupLink || "")}" placeholder="https://chat.whatsapp.com/xxxxx"></label>
    ${iconField("asWa", s.whatsappIcon || "", "LOGO WHATSAPP (opsional, default 💬)")}
    <label style="margin-top:10px">LINK INVITE DISCORD<input id="asDcLink" value="${esc(s.discordInviteLink || "")}" placeholder="https://discord.gg/xxxxx"></label>
    ${iconField("asDc", s.discordIcon || "", "LOGO DISCORD (opsional, default 🎮)")}
    <div class="notice" style="text-align:left">Kedua tombol ini muncul di Dashboard player. Diklik → muncul pop-up ajakan join + tombol JOINED yang langsung buka link di atas.</div>
  </div>
  <button class="primary full" style="margin-top:14px" onclick="saveAdminSettings()">SIMPAN SEMUA</button>`;
}
async function saveAdminSettings() {
  const body = {
    ownerWhatsApp: document.getElementById("aw").value,
    serverName: document.getElementById("asServerName").value,
    serverIp: document.getElementById("asServerIp").value,
    whatsappGroupLink: document.getElementById("asWaLink").value,
    discordInviteLink: document.getElementById("asDcLink").value,
    whatsappIcon: document.getElementById("asWa_icon").value.trim() || null,
    discordIcon: document.getElementById("asDc_icon").value.trim() || null,
  };
...
function gachaAdminHTML() {
  return `<div class="subhead"><h2>🎰 Gacha Management</h2><button class="btn" onclick="newGachaBox()">＋ TAMBAH BOX</button></div>` + STATE.gacha.map((g) => `
    <div class="card" style="margin-top:10px">
      <div class="subhead" style="margin:0 0 8px"><h3 style="margin:0">${renderIcon(g.icon)} ${esc(g.name)}</h3><div class="cactions"><button class="btn" onclick="editGachaBox('${g.id}')">EDIT BOX</button><button class="btn" onclick="deleteGachaBox('${g.id}')">HAPUS BOX</button></div></div>
      ${g.rewards.map((r) => `<div class="catitem"><div class="cicon">${renderIcon(r.icon)}</div><div class="cbody"><b>${esc(r.name)}</b><div class="muted">${esc(r.rarity)} • Chance ${r.chance}%</div></div><div class="cactions"><button class="btn" onclick="editReward('${g.id}','${r.id}')">EDIT</button><button class="btn" onclick="deleteReward('${g.id}','${r.id}')">HAPUS</button></div></div>`).join("")}
      <button class="btn full" style="margin-top:8px" onclick="newReward('${g.id}')">＋ TAMBAH REWARD</button>
    </div>`).join("");
}
function newGachaBox() {
  document.getElementById("modalbody").innerHTML = `<h2>Tambah Gacha Box</h2><label>NAMA BOX<input id="gb_name"></label>${iconField("gb", "🎁")}${iconField("gbbn", "", "BANNER / BACKGROUND (opsional)")}<div class="row"><button class="btn" onclick="closeModal()">BATAL</button><button class="primary" onclick="submitGachaBox()">SIMPAN</button></div>`;
  document.getElementById("modal").classList.remove("hide");
}
async function submitGachaBox() {
  ...
  const banner = document.getElementById("gbbn_icon").value.trim() || null;
  if (!name) return toast("Nama box wajib diisi.");
  try { await api("/admin/gacha", { method: "POST", body: { name, icon, banner } }); await refreshState(); closeModal(); admin(); toast("Box ditambahkan."); }
  catch (e) { toast(e.message); }
}
function editGachaBox(id) {
  const g = STATE.gacha.find((x) => x.id === id);
  if (!g) return;
  document.getElementById("modalbody").innerHTML = `<h2>Edit Box</h2><label>NAMA BOX<input id="gb_name" value="${esc(g.name)}"></label>${iconField("gb", g.icon)}${iconField("gbbn", g.banner || "", "BANNER / BACKGROUND (opsional)")}<div class="row"><button class="btn" onclick="closeModal()">BATAL</button><button class="primary" onclick="submitEditGachaBox('${id}')">SIMPAN</button><button class="btn" onclick="deleteGachaBox('${id}')">HAPUS</button></div>`;
  document.getElementById("modal").classList.remove("hide");
}
async function submitEditGachaBox(id) {
  ...
  const icon = document.getElementById("gb_icon").value.trim() || "🎁";
  const banner = document.getElementById("gbbn_icon").value.trim() || null;
  try { await api(`/admin/gacha/${id}`, { method: "PUT", body: { name, icon, banner } }); await refreshState(); closeModal(); admin(); toast("Box diperbarui."); }
  catch (e) { toast(e.message); }
}
...
function catalogFormFields(key, it) {
  ...
  return `<label>NAMA<input id="cf_name" value="${esc(it.name || "")}"></label><label>HARGA<input id="cf_price" type="number" value="${it.price ?? 0}"></label>${iconField("cf", it.icon || "🎁")}<label>DESKRIPSI<textarea id="cf_desc" rows="2">${esc(it.desc || "")}</textarea></label>${extra}`;
}
...
document.getElementById("regForm").onsubmit = async (e) => {
  ...
  try {
    ...
    ME = r.user;
    toast(r.ticketBonus > 0 ? `Akun dibuat: +${r.ticketBonus} Ticket GRATIS` : "Akun dibuat. Ticket gratis sudah pernah diklaim di HP ini.");
    await boot();
  } catch (e) { rgErr.textContent = e.message; }
};

// Indikator kekuatan password (live, saat registrasi)
document.getElementById("rgPass").oninput = (e) => {
  const v = e.target.value;
  const hint = document.getElementById("pwHint");
  if (!v) { hint.textContent = "Minimal 6 karakter. Biar kuat, kombinasikan huruf besar, huruf kecil, angka, dan simbol."; hint.className = "pwhint"; return; }
  let score = 0;
  if (v.length >= 6) score++;
  ...
  const level = score <= 2 ? "lemah" : score <= 3 ? "sedang" : "kuat";
  const label = { lemah: "🔴 Lemah — tambahkan huruf besar, angka, atau simbol", sedang: "🟡 Sedang — sudah lumayan, bisa lebih kuat lagi", kuat: "🟢 Kuat — password bagus!" }[level];
  hint.textContent = label;
  hint.className = "pwhint pwhint-" + level;
};
